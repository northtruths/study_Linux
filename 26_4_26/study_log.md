...
头文件源文件的拆分：只声明不定义，外部需要什么或想给外部暴露什么，就只声明什么
类在头文件是整体声明（框架必须完整），源文件才是每个模块、函数、变量的定义，对于其他可变函数、成员之类，需要什么就声明什么（用户需要接口，需要的接口中需要某些函数），像这次日志类需要策略对象，我们只声明策略的基类，具体使用哪个子类和具体的策略模块在cpp中定义并使用
